...
public interface Serializer {
     ...
     * @param object 需要序列化的对象
     * @param file 需要保存到的文件
     * @param replace 当该文件已存在时，是否替换
     * @throws IOException 写入序列化结果时抛出的异常
     */
    default void serialize(Object object, File file, boolean replace) throws IOException {
        serialize(object, file, Charset.defaultCharset().name(), replace);
    }

    /**
     * 序列化一个对象为指定编码的字符串，并将序列化后的结果存入文件中
     * @param object 需要序列化的对象
     * @param file 需要保存到的文件
     * @param encoding 编码
     * @param replace 当该文件已存在时，是否替换
     * @throws IOException 写入序列化结果时抛出的异常
     */
    default void serialize(Object object, File file, String encoding, boolean replace) throws IOException {
        ...
        try (final OutputStream outputStream = new FileOutputStream(file)) {
            serialize(object, encoding, outputStream);
        }
    }
     ...
     * @throws IOException 写入序列化结果时抛出的异常
     */
    default void serialize(Object object, File file, String encoding) throws IOException {
        serialize(object, file, encoding, true);
    }
